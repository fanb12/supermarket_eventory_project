...
async function addCustomerStatus(formData, loggedInEmployeeToken) {
  ...
  const data = await axios.post("/api/customerstatus2", formData, { headers });
  console.log(data);
...
async function getAllCustomerStatus(loggedInEmployeeToken) {
  ...
  const data = await axios.get("/api/customerstatuss2", { headers });
  console.log(data);
...
async function updateCustomerStatus(formData, loggedInEmployeeToken) {
  ...
  const data = await axios.put("/api/customerstatus2/update", formData, {
    headers,
  });
  // console.log(data)9
...
async function singleCustomerStatus(formData, loggedInEmployeeToken) {
  ...
  // console.log(formData);
  const data = await axios.get(`/api/customerstatus2/single/${formData}`, {
    headers,
  });
