...
async function addShopOutput(formData, loggedInEmployeeToken) {
  ...
  const data = await axios.post("/api/shopoutput2", formData, { headers });
  console.log(data);
  9;
  return data;
}
...
async function getAllShopOutput(loggedInEmployeeToken) {
  ...
  const data = await axios.get("/api/shopoutputs2", { headers });
  console.log(data);
...
async function updateShopOutput(formData, loggedInEmployeeToken) {
  ...
  const data = await axios.put("/api/shopoutput2/update", formData, {
    headers,
  });
  // console.log(data)
...
async function singleShopOutput(formData, loggedInEmployeeToken) {
  ...
  // console.log(formData);
  const data = await axios.get(`/api/shopoutput2/single/${formData}`, {
    headers,
  });
...
async function deleteShopOutput(shop_output_id, loggedInEmployeeToken) {
  ...
  const data = await axios.delete(`/api/shopoutput2/delete/${shop_output_id}`, {
    headers,
  });
  return data;
}
